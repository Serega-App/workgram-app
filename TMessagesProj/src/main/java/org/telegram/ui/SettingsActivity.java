...
public class SettingsActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate, ImageUpdater.ImageUpdaterDelegate, MainTabsActivity.TabFragmentDelegate, FactorAnimator.Target {
    ...
    public View createView(Context context) {
            ...
            protected void dispatchDraw(Canvas canvas) {
                if (Build.VERSION.SDK_INT >= 31 && scrollableViewNoiseSuppressor != null) {
                    ...
                    if (iBlur3SourceGlassFrosted != null && !iBlur3SourceGlassFrosted.inRecording()) {
                        ...
                        c.drawColor(getResources().getColor(R.color.mono_bg));
                ...
                if (!hasMainTabs) {
                    AndroidUtilities.drawNavigationBarProtection(canvas, this, getResources().getColor(R.color.mono_bg), navigationBarHeight);
                }
            }
        ...
        actionBar.setBackgroundColor(context.getResources().getColor(R.color.mono_bg));
        actionBar.setBackground(null);

        final ActionBarMenu menu = actionBar.createMenu();
        // Workgram: settings search removed to hide Telegram FAQ/help search results.
        searchItem = null;

        otherItem = menu.addItem(1, R.drawable.ic_ab_other);
        otherItem.addSubItem(2, R.drawable.msg_leave, getString(R.string.LogOut));

        // Workgram: search adapter disabled together with settings search.
        search = null;

        listView = new UniversalRecyclerView(this, this::fillItems, this::onClick, this::onLongClick);
        listView.adapter.setApplyBackground(false);
        listView.setSections();
        listView.setPadding(dp(12), AndroidUtilities.statusBarHeight + dp(12), dp(12), AndroidUtilities.navigationBarHeight + additionNavigationBarHeight);
        ...
        actionBarBackground = new View(context) {
            ...
            protected void onDraw(@NonNull Canvas canvas) {
                ...
                blurScrimPaint.setColor(getResources().getColor(R.color.mono_bg));
        ...
        cameraButton.setBackground(Theme.createCircleDrawable(dp(32), context.getResources().getColor(R.color.mono_bg)));
        cameraButton.setPadding(dp(2), dp(2), dp(2), dp(2));
        cameraBackground = new FrameLayout(context);
        cameraBackground.setBackground(Theme.createCircleDrawable(dp(30), context.getResources().getColor(R.color.mono_primary)));
        ...
        versionView.setTextColor(context.getResources().getColor(R.color.mono_hint));
...
//        fragmentView.addView(navigationBar, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, 0, Gravity.BOTTOM | Gravity.FILL_HORIZONTAL));
    ...
    public void updateColors() {
        final int monoBg = contentView.getContext().getResources().getColor(R.color.mono_bg);
        final int monoText = contentView.getContext().getResources().getColor(R.color.mono_text);
        final int monoHint = contentView.getContext().getResources().getColor(R.color.mono_hint);

        actionBar.setTitleColor(monoText);
        actionBar.setItemsColor(monoText, false);
        actionBar.setItemsColor(monoText, true);
        contentView.setBackgroundColor(monoBg);
        titleView.setTextColor(monoText);
        subtitleView.setTextColor(monoHint);
        if (searchItem != null) {
            searchItem.updateColor();
        }

        navigationBar.setBackground(new GradientDrawable(GradientDrawable.Orientation.TOP_BOTTOM, new int[] { Theme.multAlpha(monoBg, 0.0f), monoBg }));
    ...
    private void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        ...
        final int monoTop = getContext().getResources().getColor(R.color.mono_primary);
        final int monoBottom = getContext().getResources().getColor(R.color.mono_primary_dark);

       boolean isRu = LocaleController.getInstance().getCurrentLocaleInfo().shortName.equals("ru");

items.add(SettingCell.Factory.of(1, monoTop, monoBottom, R.drawable.settings_account,
        isRu ? "Профиль" : "Profile",
        isRu ? "Номер, имя пользователя, «О себе»" : "Phone, username, bio"));

items.add(SettingCell.Factory.of(2, monoTop, monoBottom, R.drawable.settings_chat,
        isRu ? "Чаты и оформление" : "Chats & Appearance",
        isRu ? "Обои, ночной режим, анимации" : "Wallpapers, night mode, animations"));

items.add(SettingCell.Factory.of(3, monoTop, monoBottom, R.drawable.settings_privacy,
        isRu ? "Приватность" : "Privacy",
        isRu ? "Время захода, устройства, ключи доступа" : "Last seen, devices, security"));

items.add(SettingCell.Factory.of(5, monoTop, monoBottom, R.drawable.settings_sounds,
        isRu ? "Оповещение и звуки" : "Notifications & Sounds",
        isRu ? "Звуки, звонки, счётчик сообщений" : "Sounds, calls, badges"));

items.add(SettingCell.Factory.of(6, monoTop, monoBottom, R.drawable.settings_data,
        isRu ? "Хранилище и данные" : "Data & Storage",
        isRu ? "Настройки загрузки медиафайлов" : "Media download settings"));

items.add(SettingCell.Factory.of(7, monoTop, monoBottom, R.drawable.settings_folders,
        isRu ? "Папки" : "Folders",
        isRu ? "Сортировка чатов по папкам" : "Organize chats into folders"));

items.add(SettingCell.Factory.of(8, monoTop, monoBottom, R.drawable.settings_devices,
        isRu ? "Устройства" : "Devices",
        isRu ? "Управление активными сеансами" : "Active sessions management"));

items.add(SettingCell.Factory.of(9, monoTop, monoBottom, R.drawable.settings_power,
        isRu ? "Экономия энергии" : "Power Saving",
        isRu ? "Экономия энергии при низком заряде" : "Reduce usage on low battery"));

items.add(SettingCell.Factory.of(10, monoTop, monoBottom, R.drawable.settings_language,
        isRu ? "Язык" : "Language",
        LocaleController.getCurrentLanguageName()));

        items.add(UItem.asShadow(null));

        // Workgram: Premium / Stars / Business / Gifts / Help sections removed from Settings.
        // Debug section below is kept for development builds.
    ...
    public String getVersionName() {
        try {
            ...
            return String.format(Locale.US, "Workgram v%s (%d)", pInfo.versionName, code);
        } catch (Exception e) {
            FileLog.e(e);
        }
        return null;
    ...
    private WindowInsetsCompat onApplyWindowInsets(@NonNull View v, @NonNull WindowInsetsCompat insets) {
        ...
        listView.setPadding(dp(12), statusBarHeight + dp(12), dp(12), navigationBarHeight + additionNavigationBarHeight);
    ...
    public static class AccountCell extends LinearLayout implements Theme.Colorable {
        ...
        public AccountCell(Context context, Theme.ResourcesProvider resourcesProvider) {
            ...
            textView.setTextColor(getContext().getResources().getColor(R.color.mono_text));
        ...
        public void updateColors() {
            textView.setTextColor(getContext().getResources().getColor(R.color.mono_text));
            counterView.setBackground(Theme.createRoundRectDrawable(dp(10), getContext().getResources().getColor(R.color.mono_primary)));
            arrowView.setColorFilter(new PorterDuffColorFilter(getContext().getResources().getColor(R.color.mono_hint), PorterDuff.Mode.SRC_IN));
        }
    ...
    public static class SettingCell extends LinearLayout implements Theme.Colorable {
        ...
        @Override
        public void updateColors() {
            titleView.setTextColor(getContext().getResources().getColor(R.color.mono_text));
            subtitleView.setTextColor(getContext().getResources().getColor(R.color.mono_hint));
            valueView.setTextColor(getContext().getResources().getColor(R.color.mono_primary));
